...
classes={'0':'air condition','1':'car horn','2':'children playing','3':'dog bark','4':'drilling','5':'engine idling','6':'gun_shot','7':'jack hammer','8':'siren','9':'street music'}
        ...
        for fs in os.listdir(work_dir + "/" + sub_dir):
            ...
            for (start, end, ws) in windows(sound_clip, n_frames):
                ...
                if len(signal) == ws:
                    ...
                    mfcc_spec_=librosa.feature.melspectrogram(signal,n_mels=n_mfcc)
                    mfcc_spec=librosa.amplitude_to_db(mfcc_spec_)
...
fd = open("../S2L2.0_storage/data_x_librosa_train_melspec.pkl", 'wb')
pickle.dump(features, fd)
fd2 = open("../S2L2.0_storage/data_y_librosa_train_melspec.pkl", 'wb')
pickle.dump(labels, fd2)

df=h5py.File('../S2L2.0_storage/audio_data_train_melspec.h5','w')
